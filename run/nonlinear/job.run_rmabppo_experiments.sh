...
for main_run in {1..1}; do
  ...
  N_values=(96) #(21 48 96)
  B_values=(32.0) #(7.0 16.0 32.0)
  ...
  for i in "${!N_values[@]}"; do
    ...
    bash run/nonlinear/run_rmabppo_experiment_train.sh $cdir $seed 0 $data $save_string $N $B \
    ...
    for opt_in_rate in "${opt_in_rates[@]}"; do
      for run in {1..1}; do
        ...
        bash run/nonlinear/run_rmabppo_experiment_test.sh $cdir $seed 0 $data $save_string $N $B \
