...
for main_run in {1..1}; do
  ...
  for i in "${!N_values[@]}"; do
    ...
    bash run/synthetic_continuous/run_rmabppo_experiment_train.sh $cdir $seed 0 $data $save_string $N $B \
    ...
    for opt_in_rate in "${opt_in_rates[@]}"; do
      for run in {1..1}; do
        ...
        bash run/synthetic_continuous/run_rmabppo_experiment_test.sh $cdir $seed 0 $data $save_string $N $B \
