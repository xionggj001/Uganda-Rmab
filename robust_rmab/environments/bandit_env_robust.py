...
class ContinuousStateExampleEnv(gym.Env):
    def __init__(self, N, B, seed):#, REWARD_BOUND):
        ...
        self.N = N
        self.observation_space = np.arange(S) # used in other files to get state dim
        self.action_space = np.arange(A)
        self.observation_dimension = 1
        self.action_dimension = 1
        self.action_dim_nature = N
        ...
        self.B = B
        self.init_seed = seed
        self.ground_truth = 'discrete' # 'discrete' or 'continuous'
        # ground truth and state observations may be generated by either continuous or discrete distributions
        self.model_input = 'continuous' # 'discrete' or 'continuous'
        # model input may be parameters of continuous or discrete distributions
    ...
    def update_transition_probs(self, arms_to_update):
        ...
        if self.ground_truth == 'discrete':
            ...
            self.T = continuous_T
        if self.model_input == 'discrete':
            self.model_input_T = discrete_T
        else:
            self.model_input_T = continuous_T
